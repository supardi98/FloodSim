...
app.use(
  "/result",
  express.static(path.join(process.cwd(), "result"), {
    index: "index.html",
    extensions: ["html"],
  })
);
...
app.post("/simulate", (req, res) => {
  ...
  if (!pump_log || !tiles_dir) {
    return res.status(400).json({
      status: "error",
      message: "output_tif, pump_log, and tiles_dir are required",
    });
  }
  ...
  if (!Array.isArray(rain_timeseries) || rain_timeseries.length === 0) {
    return res.status(400).json({
      status: "error",
      message: "rain_timeseries must be a non-empty array",
    });
  }
  ...
  if (!Array.isArray(pumps)) {
    return res
      .status(400)
      .json({ status: "error", message: "pumps must be an array" });
  }

  for (let i = 0; i < pumps.length; i++) {
    const p = pumps[i];
    const requiredPumpFields = [
      "in_lat",
      "in_lon",
      "out_lat",
      "out_lon",
  ...
  execFile(
    ...
    (error, stdout, stderr) => {
      ...
      if (!output_tif) {
        execFile("rm", ["result/delete_after_this.tif"], {
          cwd: process.cwd(),
        });
      }
